...
for key, value in _importer.known_modules.items():
    if key == 'ansible.module_utils.six.moves.urllib.parse':
    ...
    if key.find('ansible.module_utils.six.moves.urllib.parse.') != 0: continue
    key = key.replace('ansible.module_utils.six.moves.urllib.parse.', '')
    try:
